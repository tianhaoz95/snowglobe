...
echo "Building Snowglobe Linux App with Llama.cpp..."
# Build the app with the wrapped PATH
PATH="$WRAPPER_DIR:$PATH" flutter build linux --release --dart-define=USE_LLAMACPP=true "$@"

echo "Linux Llama.cpp build complete."
echo "Executable is located in demo/build/linux/x64/release/bundle/snowglobedemo"
